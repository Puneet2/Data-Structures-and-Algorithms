...
public class ArrayQueue<T> {
    ...
    public void enqueue(T data) {
        ...
        if (this.size == this.backingArray.length) {
            ...
            for (int i = 0; i < this.size; i++) {
                int index = (this.front + i) % this.backingArray.length;
                array[i] = this.backingArray[index];
            }
            this.front = 0;
            this.backingArray = array;

        }
        int back = (this.front + this.size) % this.backingArray.length;
    ...
    public T dequeue() {
        ...
        T removedData = this.backingArray[this.front];
    ...
    public T peek() {
        if (this.size == 0) {
            throw new NoSuchElementException("Queue was empty, can not peek");
        }
